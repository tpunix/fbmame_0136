...
void build_game_drivers(void)
{
	UINT8 *data_start = (UINT8*)&__data_start;
	UINT8 *p = (UINT8*)&__bss_start;
	struct _game_driver_info *sp;
	int count = 0;
	int i;

	while(p>data_start){
		p -= 0x10;
		if(*(UINT32*)p == 0x9a0ed15e)
			break;
	}
	count = 1;

	while(p>data_start){
		p -= sizeof(struct _game_driver_info);
		if(*(UINT32*)p != 0x9a0ed15e)
			break;
		count += 1;
	}
	sp = (struct _game_driver_info*)(p + sizeof(struct _game_driver_info));

	count += 1;
	drivers = (const game_driver**)malloc(count*sizeof(game_driver*));
