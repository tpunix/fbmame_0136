...
render_target *render_target_alloc(running_machine *machine, const char *layoutfile, UINT32 flags)
{
	...
	*nextptr = target;
	...
	for (listnum = 0; listnum < ARRAY_LENGTH(target->primlist); listnum++){
		...
		target->primlist[listnum].in_use = 0;
	}
...
const render_primitive_list *render_target_get_primitives(render_target *target)
{
	...
	while(target->primlist[listnum].in_use){
		//printf("==== primlist %p in_use! ====\n", &target->primlist[listnum]);
		osd_sleep(2000);
	}
	target->primlist[listnum].in_use = 1;
...
void render_target_free_primitives(render_primitive_list *primlist)
{
	primlist->in_use = 0;
}
